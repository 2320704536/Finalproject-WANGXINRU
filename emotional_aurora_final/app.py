# ============================================================
# Emotional Crystal — FINAL CSV EDITION (FULL VERSION)
# Part 1 / 4
# ============================================================
...
# App setup
# =========================
st.set_page_config(page_title="Emotional Crystal — Final CSV",
                   page_icon="❄️",
                   layout="wide")
...
This project generates **cinematic crystal visuals** using **emotion colors**.
...
- Enable **CSV-only color mode**: 100% force CSV colors

✔ If CSV-only is ON → **NO default colors** are used  
✔ Random mode will **fully use CSV** as well  
""")
...
# ============================================================
# Part 2 / 4 — Crystal Engine + Color System + CSV Import/Export
# ============================================================

# =========================
# Crystal Shape (core geometry)
# =========================
...
# =========================
# Soft Polygon Drawing (blur + alpha)
# =========================
...
def render_crystalmix(
    ...
    """
    df: DataFrame containing 'emotion'
    palette: emotion → (R,G,B)
    CSV-only palette already handled outside
    """
    ...
    # If empty — use palette keys only
    if not emotions:
        emotions = list(palette.keys()) if palette else ["joy"]
...
def export_palette_csv(pal):
    ...
    return buf
# ============================================================
# Part 3 / 4 — Cinematic Post-Processing + Sidebar Controls
# ============================================================

# =========================
# Cinematic Color Conversions
# =========================
...
def highlight_rolloff(img, roll):
    ...
    return np.clip(img*(1 - mask) +
                   (threshold + (img - threshold)/(1 + 4*t*mask)) * mask, 0, 1)
...
def split_tone(img, sh_rgb, hi_rgb, balance):
    ...
    return np.clip(img + sh_mask*sh_col*0.25 + hi_mask*hi_col*0.25, 0, 1)
...
# =========================
# Boost Color Saturation if too dull
# =========================
...
# =========================
# Auto-Brightness Compensation (NO errors)
# =========================
...
# ============================================================
# SIDEBAR — All Controls (CSV Palette = Section 6)
# ============================================================
...
# (Settings sent in Part 4)


# ----- (4) Cinematic Color -----
st.sidebar.header("4) Cinematic Color")
# (Sliders included in Part 4)
...
# ----- (6) Custom Palette — CSV  (YOU REQUESTED THIS POSITION) -----
...
if st.sidebar.button("Reset All", type="primary"):
    reset_all()
# ============================================================
# Part 4 / 4 — Load Data → Render Crystal → Right-Side Table
# ============================================================
...
if random_btn:
    ...
    # clear palette (CSV-only mode will use CSV only)
    st.session_state["custom_palette"] = {}
...
# 🔍 FETCH NEWS MODE
    ...
    if not key:
        ...
        raw = fetch_news(key, keyword if keyword.strip() else "aurora")
        if not raw.empty:
            # Convert text for sentiment
            raw["text"] = raw["text"].fillna("")

            # Keep URL for clickable table
            raw["url"] = raw["source"].copy()
        df = raw.copy()


# DEFAULT DEMO (when df is empty)
if df.empty:
    df = pd.DataFrame({
        "text": [
...
selected_labels = st.sidebar.multiselect(
    "Selected Emotions", option_labels, default=option_labels
)
...
# =========================================================
# Prepare Crystal Engine Controls
# =========================================================
st.sidebar.header("3) Crystal Engine")
...
bg_custom = st.sidebar.color_picker("Background Color", "#000000")
def _hex_to_rgb(x): return tuple(int(x[i:i+2], 16) for i in (1,3,5))
...
exp = st.sidebar.slider("Exposure", -0.2, 1.8, DEFAULTS["exp"], 0.01)
...
roll = st.sidebar.slider("Highlight Roll-off", 0.0, 1.5, DEFAULTS["roll"])

# White balance
st.sidebar.subheader("White Balance")
temp = st.sidebar.slider("Temperature", -1.0, 1.0, 0.0)
tint = st.sidebar.slider("Tint", -1.0, 1.0, 0.0)

# Split toning
st.sidebar.subheader("Split Toning")
sh_r = st.sidebar.slider("Shadows R", 0.0, 1.0, DEFAULTS["sh_r"])
sh_g = st.sidebar.slider("Shadows G", 0.0, 1.0, DEFAULTS["sh_g"])
sh_b = st.sidebar.slider("Shadows B", 0.0, 1.0, DEFAULTS["sh_b"])

hi_r = st.sidebar.slider("Highlights R", 0.0, 1.0, DEFAULTS["hi_r"])
hi_g = st.sidebar.slider("Highlights G", 0.0, 1.0, DEFAULTS["hi_g"])
hi_b = st.sidebar.slider("Highlights B", 0.0, 1.0, DEFAULTS["hi_b"])
...
target_mean   = st.sidebar.slider("Target Mean",   0.30, 0.70, DEFAULTS["target_mean"])
abc_strength  = st.sidebar.slider("Remap Strength",0.0, 1.0,   DEFAULTS["abc_strength"])
abc_black     = st.sidebar.slider("Black Point %", 0.00, 0.20, DEFAULTS["abc_black"])
abc_white     = st.sidebar.slider("White Point %", 0.80, 1.00, DEFAULTS["abc_white"])
abc_max_gain  = st.sidebar.slider("Max Gain",      1.0, 3.0,   DEFAULTS["abc_max_gain"])


# =========================================================
# LEFT — Render Crystal Image
# =========================================================
...
# =========================================================
# RIGHT — Data Table with CLICKABLE Links
# =========================================================
with right:
    st.subheader("📊 Data & Emotion Mapping (Clickable News Links)")
    ...
    if "url" in df2.columns:
        df2["link"] = df2["url"].apply(
            lambda x: f"<a href='{x}' target='_blank'>Open</a>" if isinstance(x, str) and x.strip() else ""
        )

    # Show HTML table (streamlit dataframe cannot show clickable HTML)
        ...
        for _, r in df.iterrows():
            ...
            for c in df.columns:
                ...
                if isinstance(cell, str) and cell.startswith("<a"):
                    ...
                    row_html += f"<td>{escape(str(cell))}</td>"
            rows += f"<tr>{row_html}</tr>"
        return f"<table>{header}{rows}</table>"

    display_cols = ["text", "emotion_display", "compound", "link"]
    st.markdown(html_table(df2[display_cols]), unsafe_allow_html=True)
