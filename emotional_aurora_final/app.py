好的！我把你“极光版”左侧的所有功能完整保留下来（NewsAPI 获取、情绪映射、Corona 引擎控制、电影级调色 + 自动亮度、调色板、下载等），并新增一种**渲染器**可选：**Ribbon Flow（丝带流纹）**，能生成你给的那两张蓝白流动感/版画质感的风格。默认就用 Ribbon Flow；你也可以切回原来的 **Aurora (Corona)**。

下面是**完整 app.py（高亮版 + Ribbon Flow）**，直接替换即可：
