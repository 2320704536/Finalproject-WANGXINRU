...
# =========================
# Base emotion colors (planet-like, then can be graded)
# =========================
DEFAULT_RGB = {
...
THEMES = {
    "Deep Night": ((0.03,0.04,0.10), (0.0,0.0,0.0)),
    "Polar Twilight": ((0.08,0.09,0.18), (0.0,0.0,0.0)),
    "Dawn Haze": ((0.12,0.10,0.14), (0.0,0.0,0.0)),
}
...
def render_corona_layer(width,height,rng,color_rgb,swirl=1.0,detail=0.8,blur_px=6):
    ...
    base = (0.60 + 0.50*noise) * np.clip(stripe,0,1)  # slightly brighter base
...
def gamma_correct(img, g):     return np.clip(img ** (1.0/max(g,1e-6)), 0, 1)
...
def split_tone(img, sh_rgb, hi_rgb, balance):
    ...
    return np.clip(img + sh*sh_col*0.25 + hi*hi_col*0.25, 0, 1)

def apply_bloom(img, radius=6.0, intensity=0.6):
    pil = Image.fromarray((np.clip(img,0,1)*255).astype(np.uint8), "RGB")
    if radius > 0:
        b = np.array(pil.filter(ImageFilter.GaussianBlur(radius=radius))).astype(np.float32)/255.0
        return np.clip(img*(1-intensity) + b*intensity, 0, 1)
    return img

def apply_vignette(img, strength=0.22):
    h,w,_ = img.shape
...
def ensure_colorfulness(img, min_sat=0.18, boost=1.22):
    r,g,b = img[:,:,0], img[:,:,1], img[:,:,2]
...
# =========================
# Cinematic palette presets
# =========================
CINEMATIC_PRESETS = {
    "Planetary (Soft)": {"mult":(1.00,1.00,1.00),"sat":1.00,"temp":0.00,"tint":0.00},
    "Cinematic Cool":  {"mult":(0.95,1.02,1.12),"sat":1.08,"temp":-0.20,"tint":0.03},
    "Cinematic Warm":  {"mult":(1.10,1.03,0.95),"sat":1.08,"temp": 0.22,"tint":-0.02},
    "Neon Arctic":     {"mult":(0.92,1.05,1.18),"sat":1.25,"temp":-0.32,"tint":0.06},
    "Sunset Storm":    {"mult":(1.16,1.04,0.92),"sat":1.22,"temp": 0.28,"tint":0.06},
    "Pastel Dream":    {"mult":(1.03,1.03,1.03),"sat":0.94,"temp": 0.05,"tint":0.05},
    "Deep Space":      {"mult":(0.96,0.99,1.06),"sat":0.98,"temp":-0.10,"tint":0.00},
}
...
def jitter_emotion_color(rgb, emo_key, band_idx, amount=0.07):
    # emotion-wise + band-wise deterministic jitter → richer, less similar colors
    seed = (abs(hash(emo_key)) + band_idx*7919) % (2**32)
...
def render_engine(df, palette, theme_name, width, height, seed, blend_mode, bands, swirl, detail, blur, bg_brightness):
    ...
    for emo in emotions:
        ...
        for bi in range(max(1,int(bands))):
            emo_rgb = np.array(jitter_emotion_color(raw_rgb, emo, bi, amount=0.08))/255.0
...
# =========================
# Auto Brightness Compensation (Always Bright)
# =========================
def auto_brightness_compensation(img_arr, target_mean=0.55, strength=0.9,
                                 black_point_pct=0.05, white_point_pct=0.998,
                                 max_gain=3.0):
    ...
    blend = 0.65*strength
    Y_mix = (1-blend)*Y2 + blend*np.clip(Y_final*gain,0,3.0)
...
def _label_emotion(e: str) -> str:
    ...
    r,g,b = base_palette.get(e,(0,0,0)); return f"{e} (Custom {r},{g},{b})"
...
detail = st.sidebar.slider("Detail Level", 0.2, 1.2, 0.90, 0.05)
blur = st.sidebar.slider("Blur (px)", 0.0, 14.0, 5.0, 0.5)
theme_name = st.sidebar.selectbox("Background Theme", list(THEMES.keys()), index=0)
bg_brightness = st.sidebar.slider("Background Brightness", 0.8, 2.0, 1.2, 0.05)
...
palette_mode = st.sidebar.selectbox("Palette Preset", list(CINEMATIC_PRESETS.keys()),
                                    index=list(CINEMATIC_PRESETS.keys()).index("Neon Arctic"))
...
# ---- 5) Auto Brightness (Always on by default, tuned brighter)
st.sidebar.header("5) Auto Brightness Compensation")
auto_bright = st.sidebar.checkbox("Enable Auto Brightness", value=True)
target_mean = st.sidebar.slider("Target Mean Luminance", 0.35, 0.65, 0.58, 0.01)
abc_strength = st.sidebar.slider("Remap Strength", 0.0, 1.0, 0.90, 0.05)
abc_black = st.sidebar.slider("Black Point Percentile", 0.00, 0.10, 0.05, 0.005)
abc_white = st.sidebar.slider("White Point Percentile", 0.95, 1.00, 0.998, 0.001)
abc_max_gain = st.sidebar.slider("Max Gain", 1.0, 4.0, 3.0, 0.1)

# ---- 6) Custom Palette (RGB) + CSV
...
with st.sidebar.expander("Add Custom Emotion (RGB 0–255)", False):
    c1,c2,c3,c4 = st.columns([1.8,1,1,1])
    name = c1.text_input("Emotion")
    r = c2.number_input("R", 0, 255, 210)
    g = c3.number_input("G", 0, 255, 190)
    b = c4.number_input("B", 0, 255, 140)
    if st.button("Add"):
        add_custom_emotion(name, r, g, b)
    show = st.session_state.get("custom_palette", {})
    if show:
        st.dataframe(pd.DataFrame(
            [{"emotion":k,"r":v[0],"g":v[1],"b":v[2]} for k,v in show.items()]
...
with left:
    ...
    if df.empty:
        ...
        img = render_engine(
            ...
            blend_mode=blend_mode, bands=bands,
            swirl=swirl, detail=detail, blur=blur,
        ...
        arr = ensure_colorfulness(arr, min_sat=0.20, boost=1.25)
        ...
        st.download_button("💾 Download PNG", data=buf, file_name="aurora_cinematic_bright.png", mime="image/png")
