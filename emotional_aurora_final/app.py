...
def jitter_emotion_color(rgb, emo_key, amount=0.03):
...
# =============== NEW: Palette Diversifier (to increase color variety) ===============
...
def hsv_to_rgb_tuple(hsv):
    ...
    return (int(np.clip(r*255,0,255)), int(np.clip(g*255,0,255)), int(np.clip(b*255,0,255)))

def diversify_palette(palette: dict, hue_spread=0.08, sat_floor=0.35, val_floor=0.45):
    """
    Spread hues using golden-angle offsets, enforce minimum saturation/value floors.
    Keeps one fixed color per emotion (deterministic, emotion-keyed).
    """
    ...
    for i, k in enumerate(keys):
        ...
        # deterministic offset (per emotion) to avoid clustering
        # using hash and golden ratio
        seed = (abs(hash(k)) % 10000) / 10000.0
        h = (h + (seed*GOLDEN*hue_spread)) % 1.0

        # enforce floors to keep colors lively & bright
        s = max(s, sat_floor)
        v = max(v, val_floor)

        diversified[k] = hsv_to_rgb_tuple((h,s,v))
    return diversified
...
def render_engine(df,palette,theme_name,width,height,seed,blend_mode,bands,swirl,detail,blur,bg_brightness):
    ...
    for emo in emotions:
        ...
        # slight, deterministic micro-variation within the same emotion bands for texture (not hue shift)
        emo_rgb = np.array(jitter_emotion_color(raw_rgb, emo, amount=0.02))/255.0
...
# NEW: color variety knobs
hue_spread = st.sidebar.slider("Hue Spread (variety)", 0.00, 0.20, 0.08, 0.005)
sat_floor = st.sidebar.slider("Saturation Floor", 0.00, 0.80, 0.38, 0.01)
val_floor = st.sidebar.slider("Value Floor", 0.00, 0.80, 0.50, 0.01)
...
with left:
    ...
    if df.empty:
        ...
        # 1) Base palette + cinematic preset
        working_palette, preset_temp, preset_tint = apply_palette_preset(get_active_palette(), palette_mode)
        # 2) Diversify hues to avoid looking too similar across emotions
        working_palette = diversify_palette(working_palette, hue_spread=hue_spread, sat_floor=sat_floor, val_floor=val_floor)
